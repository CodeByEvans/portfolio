...
export default function AboutSection() {
  ...
  const timelineItems = [
    {
      ...
      description: "Creando aplicaciones web escalables",
      detail:
        "Trabajo con React, Next.js, TypeScript, Node.js y PostgreSQL. Implemento arquitecturas serverless y microservicios.",
      icon: "💻",
      color: "cyan",
    },
    {
  ...
  export const AboutSection = (index) => {
    ...
    <section className="relative bg-gradient-to-b from-black via-gray-900 to-black">
      ...
      <div className="text-center py-20 px-4">
        ...
        <p className="text-gray-400 text-lg max-w-2xl mx-auto">
          Scroll para descubrir mi historia profesional
        </p>
      </div>

      {/* Contenedor del timeline con scroll */}
      <div ref={containerRef} className="relative min-h-[200vh] px-4">
        {/* Timeline Container */}
      ...
      <motion.div
        className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-gray-800 border border-cyan-500 rounded-full px-4 py-2 text-sm text-cyan-400 font-mono"
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
      >
        Progreso: {Math.round(scrollYProgress.get() * 100)}%
      </motion.div>
    </section>
  );
}
