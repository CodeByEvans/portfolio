  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        {/* Navegación Desktop */}
        <nav className="hidden md:flex items-center gap-8 border p-2 rounded-full">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-x text-muted-foreground hover:text-primary transition-all duration-200"
            >
              {item.label}
            </Link>
          ))}
