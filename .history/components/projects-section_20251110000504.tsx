                            ...
                            <div className="flex flex-wrap gap-2 pt-4">
                              {project.tags.map((tag, i) => (
                                <Badge
                                  key={i}
                                  variant="secondary"
                                  className="px-3 py-1 bg-background/80 border border-border font-mono text-xs transition-all duration-300 hover:scale-105 hover:bg-primary/10"
                                >
                                  {tag}
                                </Badge>
                              ))}
              ...
              {projects.map((_, index) => (
                <motion.div
                  style={{
                    opacity:
                      1 -
                      Math.abs(index / (projects.length - 1) - scrollProgress),
                    scale:
                      0.95 +
                      0.05 *
                        (1 -
                          Math.abs(
                            index / (projects.length - 1) - scrollProgress
                          )),
                    y: 50 * (index / (projects.length - 1) - scrollProgress),
                  }}
                />
              ))}
            </div>
