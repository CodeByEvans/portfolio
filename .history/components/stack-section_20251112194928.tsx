    ...
    <section>
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 gap-4 max-w-6xl mx-auto">
        {Sections.map((section, index) => (
          <Card key={index} className="p-4 w-100 mx-auto px-0">
            <div className="flex flex-col justify-between items-center text-3xl overflow-hidden gap-4 h-max">
              <h2>{section.title}</h2>
              <ParallaxItems baseVelocity={-2}>{section.items}</ParallaxItems>
              {showDescription === section.title && (
                <p className="text-base px-8 text-center">
                  {section.description}
                </p>
              )}
              <button onClick={() => setShowDescription(section.title)}>
                Ver más
              </button>
            </div>
          </Card>
